...
public class BlockWiseThread implements Runnable {
	...
	public BlockWiseThread(float[][] a, float[][] b, float[][] c, int left, int right, int rowSize) {
		A = a;
		B = b;
		C = c;
		//this.row = row;
	...
	/*
	public static void multiplyBlockWise(float[][] a, float[][] b, float[][] c, int rowSize) throws InterruptedException {
		
		int thread_count = 4;
		List<Thread> thread_list = new ArrayList<>();
		for (int i = 0; i < rowSize; ++i) {
			BlockWiseThread block = new BlockWiseThread(a, b, c, i, rowSize);
	...
	*/
	...
	public static void multiplyBlockWise(float[][] a, float[][] b, float[][] c, int rowSize) throws InterruptedException {
		...
		int eachBlock = rowSize / thread_count;
